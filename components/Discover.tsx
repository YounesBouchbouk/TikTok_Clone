...
const Discover = () => {
  ...
  return (
    <div className='xl:border-b-2 xl:border-black pb-6 '>
        <p className='text-gray-500 hidden xl:block font-semibold mt-3'>
            Popular Topics
        </p>
                ...
                topics.map(item => (
                    <Link href={`/?topic=${item.name}`}>
                        <div className={topic === item.name ? activeTopicStyle  : topicStyle}>
                            ...
                            <span className='text-bold text-md hidden xl:block capitalize'>
                                {item.name}
                            </span>
                        </div>
                    </Link>
